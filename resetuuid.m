...
// gift/panels から全ギフト（id/名前/コイン/panel_type/reason_id）を収集（名前で選ぶ用）
static NSArray<NSDictionary *> *mrv_collect_gifts(NSDictionary *panelsJson) {
    NSMutableArray *out = [NSMutableArray array];
    NSMutableSet *seen = [NSMutableSet set];
    id panels = [panelsJson isKindOfClass:NSDictionary.class] ? panelsJson[@"panels"] : nil;
    if (![panels isKindOfClass:NSArray.class]) return out;
    for (id p in panels) {
        if (![p isKindOfClass:NSDictionary.class]) continue;
        id pt  = p[@"panel_type"] ?: p[@"type"] ?: p[@"tab_type"];
        id rid = p[@"reason_id"] ?: p[@"panel_reason_id"];
        ...
        for (id g in gifts) {
            ...
            id gid = g[@"gift_id"] ?: g[@"id"];
            if (!gid) continue;
            NSString *gidS = [NSString stringWithFormat:@"%@", gid];
            if ([seen containsObject:gidS]) continue;
            [seen addObject:gidS];
            id nm   = g[@"name"] ?: g[@"title"] ?: g[@"gift_name"] ?: @"";
            id coin = g[@"coin_num"] ?: g[@"coin"] ?: g[@"price"] ?: g[@"point"];
            [out addObject:@{
                @"id":         gidS,
                @"name":       [NSString stringWithFormat:@"%@", nm],
                @"coin":       coin ? [NSString stringWithFormat:@"%@", coin] : @"",
                @"panel_type": pt  ? [NSString stringWithFormat:@"%@", pt]  : @"",
                @"reason_id":  rid ? [NSString stringWithFormat:@"%@", rid] : @"",
            }];
        }
    }
...
- (void)enterFlow;
- (void)runMissionAll;
- (void)showGiftPicker:(NSString *)live gifts:(NSArray *)gifts;
- (void)doGiftLive:(NSString *)live gift:(NSString *)gid ptype:(NSString *)pt reason:(NSString *)rid count:(NSInteger)n;
...
- (void)giftFlow {
    [self askText:@"配信URL または live_id" placeholder:@"https://... / cFb7..." completion:^(NSString *u) {
        ...
        NSArray *accts = mrv_load_accounts();
        if (!accts.count) { [self alert:@"エラー" msg:@"アカウントがありません（accounts.json 空）"]; return; }
        // 1件目の垢でギフト一覧を取得 → 名前で選ばせる
        dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
            NSInteger http = 0;
            NSDictionary *panels = mrv_api(accts[0], @"GET",
                [@"/api/gift/panels?live_id=" stringByAppendingString:live], nil, &http);
            NSArray *gifts = mrv_collect_gifts(panels);
            dispatch_async(dispatch_get_main_queue(), ^{
                if (!gifts.count) {
                    [self alert:@"取得失敗" msg:[NSString stringWithFormat:@"ギフト一覧を取得できません（HTTP %ld）", (long)http]];
                    return;
                }
                [self showGiftPicker:live gifts:gifts];
            });
        });
    }];
}
- (void)showGiftPicker:(NSString *)live gifts:(NSArray *)gifts {
    UIAlertController *ac = [UIAlertController alertControllerWithTitle:@"ギフトを選択（名前で）"
        message:[NSString stringWithFormat:@"%lu 件", (unsigned long)gifts.count]
        preferredStyle:UIAlertControllerStyleActionSheet];
    for (NSDictionary *g in gifts) {
        NSString *nm = [g[@"name"] length] ? g[@"name"] : g[@"id"];
        NSString *coin = [g[@"coin"] length] ? [NSString stringWithFormat:@"（%@）", g[@"coin"]] : @"";
        [ac addAction:[UIAlertAction actionWithTitle:[NSString stringWithFormat:@"%@%@", nm, coin]
            ...
            [self askText:@"個数 (count)" placeholder:@"1" completion:^(NSString *c) {
                ...
                dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
                    [self doGiftLive:live gift:g[@"id"] ptype:g[@"panel_type"] reason:g[@"reason_id"] count:n];
                });
            }];
        }]];
    }
...
- (void)doGiftLive:(NSString *)live gift:(NSString *)gid ptype:(NSString *)pt reason:(NSString *)rid count:(NSInteger)n {
    NSArray *accts = mrv_load_accounts();
    NSMutableString *log = [NSMutableString stringWithFormat:@"live=%@ gift=%@ pt=%@ rid=%@ count=%ld\n\n",
        live, gid, pt ?: @"", rid ?: @"", (long)n];
    int okc = 0;
    for (NSDictionary *a in accts) {
        ...
        NSDictionary *body = @{ @"count": [@(n) stringValue], @"gift_id": gid, @"live_id": live,
                                @"message": @"", @"panel_reason_id": (rid ?: @""), @"panel_type": (pt ?: @"") };
        NSString *msg = nil;
...
// ==== DeviceCheck / App Attest 観測（改変せず“呼ばれたか”をログするだけ）====
static void dc_log(NSString *line) {
    NSString *p = docs_path(@"devicecheck.txt");
    NSString *entry = [NSString stringWithFormat:@"%@  %@\n", [NSDate date], line];
    NSString *prev = [NSString stringWithContentsOfFile:p encoding:NSUTF8StringEncoding error:nil];
    if (prev.length > 128 * 1024) prev = @"";
    [(prev ? [prev stringByAppendingString:entry] : entry)
        writeToFile:p atomically:YES encoding:NSUTF8StringEncoding error:nil];
    L(@"[devicecheck] %@", line);
}
// DCDevice -generateTokenWithCompletionHandler:
static void (*g_orig_dc_token)(id, SEL, id) = NULL;
static void my_dc_token(id self, SEL _cmd, void (^completion)(NSData *, NSError *)) {
    dc_log(@"DCDevice generateToken 呼び出し");
    void (^wrap)(NSData *, NSError *) = ^(NSData *tok, NSError *err) {
        dc_log([NSString stringWithFormat:@"DCDevice token len=%lu err=%@",
            (unsigned long)tok.length, err ? err.localizedDescription : @"nil"]);
        if (completion) completion(tok, err);
    };
    if (g_orig_dc_token) g_orig_dc_token(self, _cmd, wrap);
}
// DCAppAttestService -generateKeyWithCompletionHandler:
static void (*g_orig_aa_genkey)(id, SEL, id) = NULL;
static void my_aa_genkey(id self, SEL _cmd, void (^completion)(NSString *, NSError *)) {
    dc_log(@"AppAttest generateKey 呼び出し");
    void (^wrap)(NSString *, NSError *) = ^(NSString *kid, NSError *err) {
        dc_log([NSString stringWithFormat:@"AppAttest keyId=%@ err=%@", kid ?: @"nil", err ? err.localizedDescription : @"nil"]);
        if (completion) completion(kid, err);
    };
    if (g_orig_aa_genkey) g_orig_aa_genkey(self, _cmd, wrap);
}
// DCAppAttestService -attestKey:clientDataHash:completionHandler:
static void (*g_orig_aa_attest)(id, SEL, id, id, id) = NULL;
static void my_aa_attest(id self, SEL _cmd, NSString *kid, NSData *hash, void (^completion)(NSData *, NSError *)) {
    dc_log([NSString stringWithFormat:@"AppAttest attestKey keyId=%@ hashLen=%lu", kid ?: @"nil", (unsigned long)hash.length]);
    if (g_orig_aa_attest) g_orig_aa_attest(self, _cmd, kid, hash, completion);
}
// DCAppAttestService -generateAssertion:clientDataHash:completionHandler:
static void (*g_orig_aa_assert)(id, SEL, id, id, id) = NULL;
static void my_aa_assert(id self, SEL _cmd, NSString *kid, NSData *hash, void (^completion)(NSData *, NSError *)) {
    dc_log([NSString stringWithFormat:@"AppAttest generateAssertion keyId=%@ hashLen=%lu", kid ?: @"nil", (unsigned long)hash.length]);
    if (g_orig_aa_assert) g_orig_aa_assert(self, _cmd, kid, hash, completion);
}
static void install_devicecheck_logger(void) {
    static BOOL done = NO; if (done) return; done = YES;
    Class dc = objc_getClass("DCDevice");
    if (dc) {
        Method m = class_getInstanceMethod(dc, @selector(generateTokenWithCompletionHandler:));
        if (m) { g_orig_dc_token = (void(*)(id,SEL,id))method_getImplementation(m);
                 method_setImplementation(m, (IMP)my_dc_token); dc_log(@"hook: DCDevice.generateToken 設置"); }
    } else dc_log(@"DCDevice クラス無し（DeviceCheck未使用の可能性）");
    Class aa = objc_getClass("DCAppAttestService");
    if (aa) {
        Method m1 = class_getInstanceMethod(aa, @selector(generateKeyWithCompletionHandler:));
        if (m1) { g_orig_aa_genkey = (void(*)(id,SEL,id))method_getImplementation(m1);
                  method_setImplementation(m1, (IMP)my_aa_genkey); }
        Method m2 = class_getInstanceMethod(aa, @selector(attestKey:clientDataHash:completionHandler:));
        if (m2) { g_orig_aa_attest = (void(*)(id,SEL,id,id,id))method_getImplementation(m2);
                  method_setImplementation(m2, (IMP)my_aa_attest); }
        Method m3 = class_getInstanceMethod(aa, @selector(generateAssertion:clientDataHash:completionHandler:));
        if (m3) { g_orig_aa_assert = (void(*)(id,SEL,id,id,id))method_getImplementation(m3);
                  method_setImplementation(m3, (IMP)my_aa_assert); }
        dc_log(@"hook: DCAppAttestService 設置");
    } else dc_log(@"DCAppAttestService クラス無し（App Attest未使用の可能性）");
}
...
static void reset_gate(void) {
    @autoreleasepool {
        ...
        install_net_logger();
        // DeviceCheck / App Attest の呼び出し観測（devicecheck.txt に記録）
        install_devicecheck_logger();
